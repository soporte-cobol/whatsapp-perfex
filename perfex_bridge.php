<?php
/**
 * Bridge de Emergencia V4 - Infalible
 */
...
switch ($action) {
    case 'get_customer_by_phone':
        ...
        $last7 = substr($phone, -7); // 7400000
        
        // Buscamos en contactos (que es donde está Nicolás)
        ...
        $response = $result ? array_merge($result, ['found' => true]) : ['found' => false, 'tried' => $last7];
        break;
    ...
    case 'get_customer_by_vat':
        ...
        $sql = "SELECT cl.userid as customerId, c.id as contactId, c.firstname, c.lastname, cl.company, c.email, cl.vat 
                FROM tblclients cl 
                JOIN tblcontacts c ON cl.userid = c.userid 
    ...
    case 'get_invoices':
        ...
        $sql = "SELECT id, number, total, status, hash FROM tblinvoices WHERE clientid = $cid AND status != 2 ORDER BY date DESC";
        $res = mysqli_query($conn, $sql);
